...
final class Tower
{
    ...
    {
        if (true === isset(self::$towers[$name])) {
            return self::$towers[$name];
        }

        return self::$towers[$name] = new self($name);
    }
    ...
    public function getExclusiveWeight(): int
    {
        return $this->weight;
    }
...
$towers = array_map(static function (string $tower): Tower {
    ...
    if (null !== $childrenInfo) {
        $children = explode(', ', $childrenInfo);
    }

    if (1 !== preg_match('/^([a-z]+) \(([\d]+)\)$/', $towerInfo, $match)) {
        throw new RuntimeException('Unable to parse tower');
    }

    $towerObj = Tower::make($match[1]);

    $towerObj->setWeight((int) $match[2]);

    foreach ($children as $child) {
        $child = Tower::make($child);

        $child->setParent($towerObj);
        $towerObj->addChild($child);
    }

    return $towerObj;
}, $input);

function verifyWeights(Tower $tower): ?int
{
    $children = $tower->getChildren();
    ...
    foreach ($children as $child) {
        $result = verifyWeights($child);

        if (null !== $result) {
            return $result;
        }

        $weights[$child->getName()] = $child->getInclusiveWeight();
    }

    if (1 === count(array_unique($weights))) {
        return null;
    }

    $oddWeight = array_search(1, array_count_values($weights), true);
    $oddName = array_search($oddWeight, $weights, true);
    $oddTower = Tower::make($oddName);

    unset($weights[$oddName]);

    $expectedWeight = current($weights);
    $weightDiff = $expectedWeight - $oddWeight;
...
echo $base->getName().PHP_EOL;
echo verifyWeights($base).PHP_EOL;
