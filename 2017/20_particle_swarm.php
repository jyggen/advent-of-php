...
function manhattan_distance(int $x, int $y, int $z): int
{
...
$particles = explode("\n", $input);
$particles = array_map(static function (string $particle): array {
    [$p, $v, $a] = explode(', ', $particle);

    return [
        'p' => array_map('\intval', explode(',', substr($p, 3, -1))),
        'v' => array_map('\intval', explode(',', substr($v, 3, -1))),
        'a' => array_map('\intval', explode(',', substr($a, 3, -1))),
    ];
}, $particles);

$particlesPartOne = $particles;

for ($i = 0; $i < 1337; ++$i) {
    foreach (array_keys($particlesPartOne) as $key) {
        $particlesPartOne[$key]['v'][0] += $particlesPartOne[$key]['a'][0];
        $particlesPartOne[$key]['v'][1] += $particlesPartOne[$key]['a'][1];
        $particlesPartOne[$key]['v'][2] += $particlesPartOne[$key]['a'][2];
        $particlesPartOne[$key]['p'][0] += $particlesPartOne[$key]['v'][0];
        $particlesPartOne[$key]['p'][1] += $particlesPartOne[$key]['v'][1];
        $particlesPartOne[$key]['p'][2] += $particlesPartOne[$key]['v'][2];
    }
}

$closest = array_reduce(array_keys($particlesPartOne), static function (?int $carry, int $key) use ($particlesPartOne) {
    if (null === $carry) {
        return $key;
    }

    if (manhattan_distance(...$particlesPartOne[$key]['p']) < manhattan_distance(...$particlesPartOne[$carry]['p'])) {
        return $key;
    }
...
$particlesPartTwo = $particles;

for ($i = 0; $i < 1337; ++$i) {
    $lookup = [];

    foreach (array_keys($particlesPartTwo) as $key) {
        $particlesPartTwo[$key]['v'][0] += $particlesPartTwo[$key]['a'][0];
        $particlesPartTwo[$key]['v'][1] += $particlesPartTwo[$key]['a'][1];
        $particlesPartTwo[$key]['v'][2] += $particlesPartTwo[$key]['a'][2];
        $particlesPartTwo[$key]['p'][0] += $particlesPartTwo[$key]['v'][0];
        $particlesPartTwo[$key]['p'][1] += $particlesPartTwo[$key]['v'][1];
        $particlesPartTwo[$key]['p'][2] += $particlesPartTwo[$key]['v'][2];
        $lookup[] = implode('x', $particlesPartTwo[$key]['p']);
    }

    $unique = array_unique($lookup);

    if (count($lookup) === count($unique)) {
        continue;
    }

    $diff = array_unique(array_diff_key($lookup, $unique));

    foreach ($diff as $pos) {
        [$x, $y, $z] = array_map('\intval', explode('x', $pos));

        foreach (array_keys($particlesPartTwo) as $key) {
            if ($particlesPartTwo[$key]['p'][0] !== $x ||
                $particlesPartTwo[$key]['p'][1] !== $y ||
                $particlesPartTwo[$key]['p'][2] !== $z) {
                continue;
            }

            unset($particlesPartTwo[$key]);
        }
    }
}

echo count($particlesPartTwo).PHP_EOL;
